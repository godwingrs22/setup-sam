 ...
 *
 * @param {string} githubToken - Authentication Token to use for GITHUB Apis.
 */
async function getLatestReleaseTag(githubToken) {
  const url = "https://api.github.com/repos/aws/aws-sam-cli/releases/latest";
  const client = new httpm.HttpClient("setup-sam");
  try {
    core.info(
      "Calling getLatestReleaseTag"
    );
    //By default, GITHUB API calls will be made as unauthenticated requests. If githubToken is present, it will be made as authenticated requests.
    if (githubToken) {
      core.info(
        `Adding github token to HTTP header to authenticate GITHUB API calls: ${githubToken}`
      );
...
async function installUsingNativeInstaller(inputVersion, githubToken) {
...
async function setup() {
  ...
  const binPath = useInstaller
    ? await installUsingNativeInstaller(version, githubToken)
